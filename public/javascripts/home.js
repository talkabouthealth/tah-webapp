...
$(document).ready(function() {
	...
	$("#startConvoSmall a").click(function() {
		...
		$("#startConvoSmall").hide();
		$("#postQuestionTabContent").show();
		$("#postQuestionTabContent input[type='text']").focus();
	});
	//radio buttons for Notification Accounts
	$(".notifyType").click(function() {
		var selectedType = $(this).val();
		if (selectedType === "Twitter") {
			$("#notifyUsernamePanel").hide();
		}
		else {
			$("#notifyUsernamePanel").show();
		}
	}); 
	
	$(".ccTwitterCheckbox").click(function() {
		if (!hasTwitter && $(this).attr("checked")) {
			hasTwitter = true;
			openTwitter("sameurl");
		}
	});	
	$(".ccFacebookCheckbox").click(function() {
		if (!hasFacebook && $(this).attr("checked")) {
			hasFacebook = true;
			openFacebook("sameurl");
		}
	});	
...
function loadSimilarMember(type,divId) {
	$.get("/home/feedAjaxLoad", { feedType: type, talkerName: ''},
		function(data) {
			if(data == '')
				$(divId).hide();
			else{
...
function loadPopularTopics() {
	$.get("/home/feedPopularTopics", { },
		function(data) {
			if(data == '')
				$("#popularTopicsCnt").hide();
			else{
				$("#popularTopicsCnt").html(data);
				$("#popularTopicsCnt").show();
			}
		}
	);
}
...
function initHomeTabs() {
	$(".tabLink").click(function() {
		...
		if(id == "allFeed"){
			...
			$(".tabContent").hide();
			$.get("/home/loadCancerFeed", {feedType: feedType, csrType: csrType},
					function(data) {
